...
const generateEnglishEmailTemplate = (booking, template = 'confirmed') => {
  const speroColor = 'rgb(232, 138, 120)';
  const styles = `
  * {
    margin: 0;
...
`;
  ...
  const formatEnglishPrice = (price, serviceType) => {
    if (!price) return '€0';
  ...
  const generateEnglishJourneySection = (journey, isOutbound, price, serviceType) => {
    if (!journey || (!journey.date && !journey.time && !journey.airport)) {
      return `
        <div class="journey-info">
          <p class="journey-title" style="font-style: italic; color: #888;">
            ${isOutbound ? 'No outbound journey' : 'No return journey'}
          </p>
        </div>
      `;
    }
    ...
    return `
      <div class="journey-info">
        <div class="info-row">
          <div class="info-label">Date:</div>
          <div class="info-value">${formatEnglishDate(journey.date)}</div>
        </div>
        <div class="info-row">
          <div class="info-label">Journey type:</div>
          <div class="info-value">${isOutbound ? 'Outbound' : 'Return'}</div>
        </div>
        <div class="info-row">
          <div class="info-label">Flight time:</div>
          <div class="info-value">${journey.time || 'Not specified'}</div>
        </div>
        ${isOutbound ? `
          <div class="info-row">
            <div class="info-label">Pickup time:</div>
            <div class="info-value">${journey.pickupTime || 'To be confirmed'}</div>
          </div>
        ` : `
          <div class="info-row">
            <div class="info-label">Flight number:</div>
            <div class="info-value">${journey.flightNumber || 'Not specified'}</div>
          </div>
          <div class="info-row">
            <div class="info-label">Flight origin:</div>
            <div class="info-value">${journey.flightOrigin || 'Not specified'}</div>
          </div>
        `}
        <div class="info-row">
          <div class="info-label">Airport:</div>
          <div class="info-value">${journey.airport || 'Not specified'}</div>
        </div>
        <div class="info-row">
          <div class="info-label">Price:</div>
          <div class="info-value">${formatEnglishPrice({ sharedPrice: displayPrice }, serviceType)}</div>
        </div>
      </div>
    `;
  };
  ...
  if (template === 'rejected') {
    return `
      ...
      <body>
        <div class="header">
          <h1 style="margin:0;font-size:18px;">Booking Request Not Confirmed</h1>
          <p style="margin:3px 0;font-size:14px;">Spero Shuttle</p>
        </div>

        <div class="content-section">
          <div class="section-content">
            <p>Dear ${booking.client.firstName} ${booking.client.lastName},</p>
            <p style="margin: 15px 0;">We are sorry to inform you that your booking request for ${formatEnglishDate(booking.journey.outbound?.date || booking.journey.inbound?.date)} could not be confirmed.</p>
            <p style="margin: 15px 0;">Please feel free to contact us for more information or to make a new booking.</p>
          </div>
        </div>

        <div class="footer">
          <strong>For any questions, contact us:</strong>
          <p style="margin:3px 0;">Tel: +32 490 39 69 67</p>
          <p style="margin:3px 0;">Email: spero.navette@gmail.com</p>
          <p style="margin:3px 0;"><strong>Spero Shuttle - Your holidays at your doorstep!</strong></p>
        </div>
      </body>
      </html>
    `;
  ...
  // Template for confirmation (default)
  try {
    const hasOutbound = !!booking.journey?.outbound;
    const hasInbound = !!booking.journey?.inbound;
    const client = {
      firstName: booking.client?.firstName || 'Not specified',
      lastName: booking.client?.lastName || 'Not specified',
      phone: booking.client?.phone || 'Not specified',
      address: {
        street: booking.client?.address?.street || 'Not specified',
        number: booking.client?.address?.number || '',
        postalCode: booking.client?.address?.postalCode || 'Not specified',
        city: booking.client?.address?.city || 'Not specified'
      }
    };
    ...
    const paymentMethod = paymentMethodTranslation[booking.paymentMethod] || booking.paymentMethod || 'Not specified';
    ...
    return `
      ...
      <body>
        <div class="header">
          <h1 style="margin:0;font-size:18px;">Booking Confirmation</h1>
          <p style="margin:3px 0;font-size:14px;">Spero Shuttle</p>
        </div>
        ...
        <div class="content-section">
          ...
          <div class="section-content">
            ${hasOutbound ? `
              <div class="journey-title">Outbound</div>
              ${generateEnglishJourneySection(booking.journey.outbound, true, booking.price, booking.serviceType)}
            ` : generateEnglishJourneySection(null, true, booking.price, booking.serviceType)}
            
            ${hasInbound ? `
              <div class="journey-title">Return</div>
              ${generateEnglishJourneySection(booking.journey.inbound, false, booking.price, booking.serviceType)}
            ` : generateEnglishJourneySection(null, false, booking.price, booking.serviceType)}
          </div>
        </div>

        <div class="content-section">
          <h2 class="section-title">Booking Details & Free Options</h2>
          <div class="section-content">
            <div class="info-row">
            ...
            <div class="info-row">
              ...
              <div class="info-value">${booking.serviceType === 'private' ? 'Private Shuttle' : 'Shared Shuttle'}</div>
            </div>
            ${booking.recommendedBy ? `
            <div class="info-row">
            ...
            <div class="info-row">
              <div class="info-label">Free options:</div>
              <ul class="options-list">
                ${booking.options?.luggageCount !== undefined ? `<li>Checked luggage: ${booking.options.luggageCount}</li>` : ''}
                ${booking.options?.handLuggageCount !== undefined ? `<li>Hand luggage: ${booking.options.handLuggageCount}</li>` : ''}
                ${booking.options?.childSeatsCount !== undefined ? `<li>Child seats: ${booking.options.childSeatsCount}</li>` : ''}
                ${booking.options?.boosterSeatsCount !== undefined ? `<li>Booster seats: ${booking.options.boosterSeatsCount}</li>` : ''}
                ${booking.options?.other ? `<li>Other: ${booking.options.other}</li>` : ''}
                ${booking.options?.luggageCount === undefined && 
                  booking.options?.handLuggageCount === undefined && 
                  booking.options?.childSeatsCount === undefined && 
                  booking.options?.boosterSeatsCount === undefined && 
                  !booking.options?.other ? 
                  `<li style="color:${speroColor};font-style:italic;">No options selected</li>` : ''}
              </ul>
            </div>
            <div class="info-row">
              <div class="info-label">Total price${booking.journey.type === 'roundTrip' ? ' (round trip)' : ''}:</div>
              <div class="info-value">${formatEnglishPrice(booking.price, booking.serviceType)}</div>
              ${booking.journey.type === 'roundTrip' ? `
                <div style="font-size: 11px; color: #666;">
                  (Outbound: ${formatEnglishPrice(booking.journey.outbound.price, booking.serviceType)} + 
                   Return: ${formatEnglishPrice(booking.journey.inbound.price, booking.serviceType)})
                </div>
              ` : ''}
            </div>
            ...
            <div class="payment-info">
              <p>
                Cash payment to driver on transfer day<br>
                or<br>
                Bank transfer at least 5 working days before transfer to the following account:<br>
                Name: Spero Navette SRL<br>
                Account number: BE64 3630 0968 1852<br>
                Reference: "${client.lastName} ${client.firstName} ${client.address.postalCode} ${client.address.city}"
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="content-section">
          ...
          <div class="section-content">
            <ul class="options-list">
              <li>Please be ready 5 minutes before the indicated pickup time</li>
              <li>In case of delay or unexpected events, please contact us immediately</li>
              <li>For any booking modifications, contact us 24 hours in advance</li>
            </ul>
          </div>
        </div>

        <div class="footer">
          <strong>For any questions or modifications, contact us:</strong>
          <p style="margin:3px 0;">Tel: +32 490 39 69 67</p>
          <p style="margin:3px 0;">Email: spero.navette@gmail.com</p>
          <p style="margin:3px 0;"><strong>Spero Shuttle - Your holidays at your doorstep!</strong></p>
        </div>
    ...
    console.error('Error in English template generation:', error);
    throw error;
  }
};
